...
actor erc721_token = {
  private stable var init_minter: Principal = Principal.fromText("5gmo5-uvhcx-d4fqb-yhovm-7din3-qfkn7-3zacz-b46b2-wunfx-z5mn7-zqe");
  // Types
  ...
  // private stable var _registryPrincipalState : [(TokenIndex, Principal)] = [];
  // private var _registryPrincipal : HashMap.HashMap<TokenIndex, Principal> = HashMap.fromIter(_registryPrincipalState.vals(), 0, ExtCore.TokenIndex.equal, ExtCore.TokenIndex.hash);
  ...
  custodians := TrieSet.put(custodians,init_minter,Principal.hash(init_minter),Principal.equal);

  //State functions
  system func preupgrade() {
    _registryState := Iter.toArray(_registry.entries());
    // _registryPrincipal := Iter.toArray(_registryPrincipal.entries());
  ...
  system func postupgrade() {
    _registryState := [];
    // _registryPrincipalState = [];
    _allowancesState := [];
    _tokenMetadataState := [];
    _custodiansState:= [];
  };

  // dfx canister --network=ic call nft setMinter "(principal \"2vxsx-fae\")"
	public shared(msg) func setMinter(minter : Principal) : async () {
		assert(TrieSet.mem(custodians, msg.caller, Principal.hash(msg.caller), Principal.equal));
    custodians := TrieSet.put(custodians,minter,Principal.hash(minter),Principal.equal);
		_minter := minter;
	};
	
  // comment this function for production since to match the standard pattern
  ...
  public query func thisCanister() : async Principal {
      return Principal.fromActor(erc721_token);
  };
  ...
  public shared(msg) func mintNFT(request : MintRequest) : async TokenIndex {
    ...
    // _registryPrincipal.put(token,request.to)
  ...
  public shared(msg) func transfer(request: TransferRequest) : async TransferResponse {
    if (request.amount != 1) {
		...
		if (ExtCore.TokenIdentifier.isPrincipal(request.token, Principal.fromActor(erc721_token)) == false) {
  ...
  public query func balance(request : BalanceRequest) : async BalanceResponse {
	...
	public query func bearer(token : TokenIdentifier) : async Result.Result<AccountIdentifier, CommonError> {
		if (ExtCore.TokenIdentifier.isPrincipal(token, Principal.fromActor(erc721_token)) == false) {
  ...
  // public query func getMetadataByIdx(tokenind : TokenIndex) : async Result.Result<Metadata, CommonError> {
  //   switch (_tokenMetadata.get(tokenind)) {
  //     case (?token_metadata) {
	// 			return #ok(token_metadata);
  //     };
  //     case (_) {
  //       return #err(#InvalidToken(tokenind.toText()));
  //     };
  //   };
  // };
